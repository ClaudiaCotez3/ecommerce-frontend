...
const getRefreshToken = (): string | null => {
  ...
  return localStorage.getItem(refreshConfig.refreshTokenKey) || 
 ...
 */
export const setupRefreshTokenInterceptor = (axiosInstance: AxiosInstance): void => {
  if (!refreshConfig.enabled) {
    if (process.env.NODE_ENV === 'development') {
      console.log('🔧 Refresh token interceptor disabled. Enable in refresh-token.ts when backend is ready.');
    }
    return;
  }
    ...
    async (error: AxiosError) => {
      ...
      if (error.response?.status === 401 && !originalRequest._retry) {
        ...
        try {
          ...
          const response = await axiosInstance.post(refreshConfig.refreshEndpoint, {
